...
def load_merged_environment():
    candidates=[APP_ROOT.parents[1]/".env",APP_ROOT/".env"]
    source_root=APP_ROOT/"sources"
    if source_root.is_dir():
        for source in sorted(source_root.iterdir(),key=lambda item:item.name.lower()):
            candidates.extend((source/".env",source/"backend"/".env",source/"server"/".env"))
    for candidate in candidates: load_env_file(candidate)
...
def feature_operations_html(feature):
    data=rows("""SELECT reference,title,owner,status,priority,due_date,notes
        FROM feature_operation_records WHERE feature_id=? ORDER BY due_date,id""",(feature["id"],))
    columns=(
        ("reference","Reference"),("title","Title"),("owner","Owner"),("status","Status"),
        ("priority","Priority"),("due_date","Due Date"),("notes","Notes"),
    )
    headers="".join(f'<th>{label}</th>' for _,label in columns)
    body="".join(f'<tr {record_attributes(record)}>'+"".join(f'<td>{html.escape(str(record.get(key,"")))}</td>' for key,_ in columns)+"</tr>" for record in data)
    return f'''<section class="detail"><div class="eyebrow">Operational records</div><h1>{html.escape(feature["name"])}</h1><p class="lead">{len(data)} example records with ownership, status, priority, due dates, and operating notes.</p><div class="table"><table><thead><tr>{headers}</tr></thead><tbody>{body}</tbody></table></div></section>'''
...
def navigation_links(items, selected, route_counts):
    return "".join(f'<a class="nav-item {"active" if selected and row["id"]==selected["id"] else ""}" href="{html.escape(preferred_route(row,route_counts))}"><span>{html.escape(row["name"])}</span><small>{"AI" if row["ai_route"] else row.get("operation_count") or row["evidence_count"]}</small></a>' for row in items)
...
def render(query, request_path="/"):
    ...
    feature_rows=rows("SELECT features.*,(SELECT route FROM ai_pages WHERE ai_pages.feature_id=features.id ORDER BY id LIMIT 1) AS ai_route,(SELECT COUNT(*) FROM feature_operation_records WHERE feature_operation_records.feature_id=features.id) AS operation_count,feature_navigation.tier,feature_navigation.priority FROM features JOIN feature_navigation ON feature_navigation.feature_id=features.id"+clause+" ORDER BY CASE feature_navigation.tier WHEN 'primary' THEN 1 WHEN 'secondary' THEN 2 ELSE 3 END,feature_navigation.priority DESC,features.name",params)
    ...
    if feature_id.isdigit():
        found=rows("SELECT features.*,(SELECT route FROM ai_pages WHERE ai_pages.feature_id=features.id ORDER BY id LIMIT 1) AS ai_route,(SELECT COUNT(*) FROM feature_operation_records WHERE feature_operation_records.feature_id=features.id) AS operation_count,feature_navigation.tier,feature_navigation.priority FROM features JOIN feature_navigation ON feature_navigation.feature_id=features.id WHERE features.id=? AND "+visibility_rule,(int(feature_id),)); selected=found[0] if found else None
    ...
    if source_row and not table_payload and not selected:
        ...
        if ai_pages: detail=ai_page_html(ai_pages[0])
         ...
         feature=feature_payload(selected)
         ...
         if related and related[0]["relevance_score"]>=40:
          ...
          detail=feature_operations_html(feature)
...
def openrouter_completion(page,values):
    ...
    if not api_key: raise RuntimeError("OpenRouter is not configured. Add OPENROUTER_API_KEY to a merged source .env file.")
